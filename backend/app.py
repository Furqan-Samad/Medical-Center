...
# Configure Flask with explicit template and static folders so the app
# is accessible when run from the `backend` folder but serves files from
# the workspace-level `static/` directory.
app = Flask(__name__,
...
# Folder where lab reports will be saved (absolute path inside project static)
UPLOAD_FOLDER = os.path.join(app.static_folder, 'uploads', 'reports')
...
def print_invoice(id):
    ...
    return render_template("invoice.html", patient=patient)

def send_whatsapp_pdf(phone, pdf_path, patient_name):
    ...
    # Step 1: upload media (simple direct link method)
    file_url = f"https://yourdomain.com/{pdf_path}"
...
def consultant_room():
    ...
    cursor.execute("""
        SELECT id, name, age, gender, disease_summary, is_selected
        FROM patients
        WHERE is_active = 1
        ORDER BY id DESC
    """)
    ...
    return render_template("consultant.html", patients=patients)

@app.route('/consultant_patients')
def consultant_patients_api():
    conn = get_db_connection()
    cursor = conn.cursor()

    cursor.execute("""
        SELECT id, name, age, gender, disease_summary
        FROM patients
        WHERE is_active = 1
        ORDER BY id DESC
    """)
    ...
    return redirect(url_for('consultant_room'))
...
def select_patient(id):
    ...
    cursor.execute("UPDATE patients SET is_selected = 1 WHERE id = %s", (id,))
    conn.commit()
...
@app.route('/unselect_patient/<int:id>')
def unselect_patient(id):
    conn = get_db_connection()
    cursor = conn.cursor()

    cursor.execute("UPDATE patients SET is_selected = 0 WHERE id = %s", (id,))
    conn.commit()
...
@app.route('/lab_dashboard', methods=['GET'])
@role_required(['lab', 'doctor'])
def lab_dashboard():
    if 'user_id' not in session:
        return redirect(url_for('login'))
    ...
    # Get all patients (for selection/search)
    cursor.execute("SELECT id, name, phone, age FROM patients WHERE is_active = 1")
    patients = cursor.fetchall()

    # Get latest lab reports with patient info
    cursor.execute("""
        SELECT 
            l.id,
            l.patient_id,
    ...
    return render_template("lab_window.html",
                           patients=patients,
                           reports=reports)

@app.route("/add_lab/<int:patient_id>", methods=["POST"])
def add_lab(patient_id):
    ...
    cursor.execute("""
        ...
        (patient_id, test_type, hb, wbc, platelets, rbc,
         alt, ast, bilirubin,
         creatinine, urea, bun, status)
...
if __name__ == "__main__":
    ...
    print("[RUNNING] MedPro AI System Active and listening on 0.0.0.0:5000 (accessible on LAN)")
